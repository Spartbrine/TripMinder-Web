...
export class pgSliderComponent implements ControlValueAccessor, OnInit, OnChanges, OnDestroy {
  ...
  getValueToOffset(value?: SliderValue): SliderValue {
    ...
    // TODO: using type guard, remove type cast
    return this.Range ? (normalizedValue as number[]).map(val => this.valueToOffset(val)) : this.valueToOffset(normalizedValue as number);
  }
  ...
  findClosestValue(position: number): number {
    ...
    // return the fixed
    return this.Step === null ? closest : parseFloat(closest.toFixed(this.utils.getPrecision(this.Step)));
  }
