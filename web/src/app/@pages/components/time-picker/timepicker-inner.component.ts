import { forwardRef, ChangeDetectorRef, Component, Input, OnInit, ViewChild, ViewEncapsulation } from '@angular/core';
...
@Component({
  ...
  template: `
    <div class="time-picker-panel" [@dropDownAnimation]="'bottom'">
      <div
        class="time-picker-inner"
      ...
      >
        <div class="time-picker-panel-combobox d-flex flex-row">
          <div class="select-panel" #hourListInstance *ngIf="_showHour">
          ...
          <div class="select-panel" #minuteListInstance *ngIf="_showMinute">
            <ul class="no-style no-padding text-center">
          ...
          <div class="select-panel" #secondListInstance *ngIf="_showSecond">
            <ul class="no-style no-padding text-center">
          ...
          <div class="select-panel" #ampmListInstance *ngIf="_showAMPM" (mouseover)="_overSecond()">
            <ul class="no-style no-padding text-center">
...
export class pgTimePickerInnerComponent implements OnInit, ControlValueAccessor {
  ...
  _selectAMPM(instance: HTMLElement, index: number, disabled: boolean): void {
    ...
    const tempDateString =
      moment(this.Value).format('YYYY MM DD') + ',' + moment(this.Value).format(this._format) + ' ' + this._ampmList[index].name;
  ...
  _buildHours(): void {
    ...
    for (let i = 0; i <= dayLength; i++) {
      ...
      if (dayLength == 11) {
        this._hourList[i]['name'] = moment()
          .startOf('day')
          .add(i + 1, 'hours')
          .format(this._format[0]);
        this._hourList[i]['index'] = i + 1;
      } else {
        this._hourList[i]['name'] =
          i.toString().length === 0
            ? moment()
                .startOf('day')
                .format(this._format[0])
